...
EMsgBoxResult IGraphicsWin::ShowMessageBox(const char* str, const char* title, EMsgBoxType type, EMsgBoxIcon icon, IMsgBoxCompletionHandlerFunc completionHandler)
{
  ReleaseMouseCapture();
  ...
  EMsgBoxResult result = static_cast<EMsgBoxResult>(MessageBoxW(GetMainWnd(), UTF8AsUTF16(str).Get(), UTF8AsUTF16(title).Get(), flag));
...
{
  if (confirmMsg && MessageBoxW(mPlugWnd, UTF8AsUTF16(confirmMsg).Get(), UTF8AsUTF16(msgWindowTitle).Get(), MB_YESNO | MB_ICONQUESTION) != IDYES)
  {
    return false;
  }
  DWORD inetStatus = 0;
  ...
  if (errMsgOnFailure)
  {
    MessageBoxW(mPlugWnd, UTF8AsUTF16(errMsgOnFailure).Get(), UTF8AsUTF16(msgWindowTitle).Get(), MB_OK | MB_ICONEXCLAMATION);
  }
  return false;
}
